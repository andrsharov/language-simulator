...
* [Запуск анализатора Pylint](#Запуск-анализатора-Pylint)
...
## Запуск анализатора Pylint

Анализатор Pylint требуется запускать в корневой директории проекта, там, где находится файл `.pylintrc`.
Запуск производится с помощью следующей команды:
```bash
pylint --load-plugins=pylint_django --django-settings-module=app.settings app/simulator
```